...
// 	ReadTimeout      time.Duration `yaml:"read_timeout"`       // 读超时时间
// 	WriteTimeout     time.Duration `yaml:"write_timeout"`      // 写超时时间
// 	ConnMinIdle      int           `yaml:"conn_min_idle"`      // 连接池最小空闲连接数
// 	ConnMaxOpen      int           `yaml:"conn_max_open"`      // 连接池最大连接数
// 	ConnMaxLifetime  time.Duration `yaml:"conn_max_lifetime"`  // 连接数最大生命周期
// 	ConnMaxIdleTime  time.Duration `yaml:"conn_max_idletime"`  // 连接数空闲时长
// }
